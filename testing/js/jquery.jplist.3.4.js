   ...
   $.fn.jplist = function(options){
	...
	return this.each(function() {            
		...
		function FilterClass(){
			...
			this.date_fields = this.filter_pathes.find("input[filter-type=date]");
			this.select_fields = this.filter_pathes.find("Select");
			...
			this.select_fields.change(function(){
				hndl.pagingObj.cpage = 0;
				hndl.viewObj.updateView();
			});
			...
			this.format_nume = function(val){
			
				var temp = new String(val);				
				temp = temp.replace(/[^0-9>=<.-]+/g,'');				
				return temp.toLowerCase();
			};
			this.format_num = function(val){
			
				var temp = new String(val);				
				temp = temp.replace(/[^0-9.-]+/g,'');				
				return temp.toLowerCase();
			};
			this.format_date = function(val){
			
				var temp = new String(val);				
				temp = temp.replace(/[^0-9.\/to-]+/g,'');				
				return temp.toLowerCase();
			};
			...
			//alert(f_hndl.input_fields.length);
			this.filterData = function(data){
				...
				for (var i=0; i<data.length; i++){
					...
					f_hndl.input_fields.each(function(){		
						var class_name = $(this).attr("filter");
						...
						if(filter_type == "num"){
							var val = f_hndl.format_nume($(this).val());
							if(val.indexOf(">=") == 0){
								if(Number(f_hndl.format_num(data_item[0])) >= Number(f_hndl.format_num(val))
									|| ($.trim(val) == f_hndl.format_num(val))){
								...
								if((f_hndl.format_num(data_item[0]).indexOf(val) != -1) || 
									($.trim(val) == "")){							
									item_to_add_arr.push(true);
								}
								else{
							...
							var val = f_hndl.format_date($(this).val());
							var mydate = val.split("to");
							//alert(typeof mydate[1]);
							if(typeof mydate[1] != "undefined"){
								var getlstDate = data_item[0].split(" ");
								var lstDate = f_hndl.format_date(getlstDate[0]);
								lstDate = new Date(Date.parse(lstDate.replace(/-/g, " ")));
								var startDate = new Date(Date.parse(mydate[0].replace(/-/g, " ")));
								var endDate = new Date(Date.parse(mydate[1].replace(/-/g, " ")));
								//alert(startDate + " <= " + lstDate + " >=" + endDate);
								if(lstDate >= startDate && lstDate <= endDate){
									item_to_add_arr.push(true);
								}else{
									item_to_add_arr.push(false);
								}
							}else{
								if((f_hndl.format_date(data_item[0]).indexOf(val) != -1) || 
								($.trim(val) == "")){							
									item_to_add_arr.push(true);
								}
								else{
					...
					f_hndl.select_fields.each(function(){												
						var val = f_hndl.format($(this).val());
						var class_name = $(this).attr("filter");
						var filter_index = hndl.dataObj["filter-" + class_name];
						var data_item = filter_arr[filter_index];
						//alert(typeof data_item == "undefined");
						//if(typeof data_item != "undefined"){
							if((f_hndl.format(data_item[0]).indexOf(val) != -1) || 
								($.trim(val) == "")){							
								item_to_add_arr.push(true);
		...
		function SortClass(){
			...
			this.sort_ph = $(hndl).find(options.sort_path);
			this.sort_ph.find("a").click(function(){
