...
<div class="container space">
    ...
    <div class="col-md-6 col-md-offset-3 col-xs-12">
        ...
        <p>Just enter the e-mail address you used for registration - we will send you a link to reset your password.</p>
			<div class="row ForgetPasword_Email">
				<form onsubmit="return false;">
					<div class="row">
							<div class="has-feedback">
							<input type="text" class="form-control" name="txt_email" placeholder="Your Registered Email" style="height:45px; width:100%">
                            <span class="epi-info-circled form-control-feedback hide"  aria-hidden="true"></span>
							</div>
							<br>
							<button class="btn btn-primary btn-lg" type="button" onclick="retrievePassword.submit()" style="width:100%">Send</button>
					</div>
				</form>
			</div>
            <br>
            ...
            <br>
			<div class="row ForgetPasword_SMS">
				<p>Enter your registered mobile number - we will send you a code to reset your password.</p>
				<form onsubmit="return false;">
						<div class="row">
                            ...
                            <br>
							<button class="btn btn-primary btn-lg" type="button" onclick="retrievePassword.submitSMS()" style="width: 100%;">Send</button>
						</div>
				</form>
			</div>
		</div>
...
var retrievePassword = {
	submit : function(){
		...
		if(!General.isTextEmpty(email) && General.validateEmail(email.val())){
			$.ajax({
				type : "post",
				url : "./?ac=doRetrievePassword",
				data : { email : email.val()},
				success : function(result){
					if($.trim(result) == "pass"){
						title = "Email Send Success";
						message = "We have sent you an email. Please check your email.<br>";
						buttonMessage = '<button class="btn btn-primary" onclick="loginPage()" type="button">Go To Login Page</button>';
					}else if($.trim(result) == "mail_fail"){
						title = "Oops!";
						message = "Sorry, our Mail server is currently not accessible, Please try again later.<br>";
						buttonMessage = '<button class="btn btn-primary" onclick="visitPage()" type="button">Return To Home</button>';
					}else{
						title = "Oops!";
						message = "Sorry, you are not our user yet. You may register by clicking on the button below.<br>";
						buttonMessage = '<button class="btn btn-primary" onclick="signPage()" type="button">Go To Register Page</button>';
					}
				...
				$("#main_display").html('<button class="btn btn-primary" onclick="visitPage()" type="button">Return To Home</button>');
			});
		}else{
			CheckNull($(".ForgetPasword_Email [name=txt_email]"));
			message = "Warning: You have not entered an valid email.";
	...
	submitSMS : function(){
		var mobile = $(".ForgetPasword_SMS [name=txt_mobile]");
		mobile = $.trim(mobile.val().replace(/\D/g,''));
		code = $("[name=mobile_code]").val();
		mobilefield = '<input type="hidden" name="inputMobile" value="'+mobile+'">'
		codefield = '<input type="hidden" name="inputMobileCode" value="'+code+'">';
		title = message = buttonMessage = "";
		if(mobile != ""){
			$.ajax({
				type : "post",
				url : "./?ac=doRetrievePasswordMobile",
				data : {
					code : code,
					mobile : mobile
				},
				success : function(result){
					if($.trim(result) == "pass"){
						...
						title = message = "";
						buttonMessage = $("#sms_code").html()+mobilefield+codefield;
					}else if($.trim(result) == "fail"){
						title = "<h4>Oops!</h4>";
						message = "<div class='alert alert-info' role='alert'>Sorry, the system cannot match your mobile number. You may reset your password using your email address or register by clicking on the button below.</div>";
						buttonMessage = '<button class="btn btn-primary" onclick="signPage()" type="button">Go To Register Page</button>';
					}else if(result.substring(0,6) == "Sorry."){
						title = "<h4>Oops!</h4>";
						message = "<div class='alert alert-info' role='alert'>"+result+"</div>";
						buttonMessage = '<button class="btn btn-info" onclick="reloadPage()" type="button">Retrieve Password Thru Email</button><hr>'+$("#sms_code").html()+mobilefield+codefield;
					}else{
						title = "<h4>Oops!</h4>";
			...
			CheckNull($(".ForgetPasword_SMS [name=txt_mobile]"));
...
function visitPage(){
	window.location='https://secure.easyparcel.my/pass/';
}
