...
firebase.auth().onAuthStateChanged( firebaseUser => {
	if(firebaseUser) {
							...
							var keys = Object.keys(object1);
								...
								for (var i=0; i<keys.length; i++) {
										...
										shipmentdata.once('value', snap=>{
											...
											var tr = $("<tr>").html('<td width="2%"><input type=\'checkbox\' name=\'checkall\' id=\'checkall\' onclick="AwbCheckAll()"></td><td width="15%" class="hidden-xs">'+shipKey+'</td><td width="26%" class="hidden-xs">'+object2.orderDate+'</td><td width="26%" class="hidden-xs">'+object2.status+'</td><td width="20%" class="hidden-xs">'+object2.totalPrice.toFixed(2)+'</td><td width="6%" class="hidden-xs"><a class="btn btn-info btn-xs" href="javascript:showDetail(\''+shipKey+'\')"><i class="glyphicon glyphicon-triangle-right" aria-hidden="true"></i></a></td>');
											$("#table1").append(tr);
				...
				window.location.href = '<?=base_url('order/')?>' + id + '/2';
