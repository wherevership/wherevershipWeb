   ...
   ?>
	<tr>
		...
		<td width="15%">
			...
			<a href="javascript:toDelete('<?=base_url('admin/driver_delete/'.$v['id'])?>');" class="btn btn-danger btn-xs">Delete</a>
...
<div class="modal fade" id="driverDetail" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
	...
	url = "<?=base_url('admin/UpdateUserStatus')?>";
			...
			$.ajax({
				...
				success: function(result) {
					console.log(result);
					window.location.href = '<?=base_url("admin/driver_list")?>';
...
function addDriver() {
	window.location.href = '<?=base_url("admin/add_driver")?>';
}
...
var orderlist = {
	...
	search : function(){
		url = "<?=base_url('admin/searchDriver')?>";
...
function bulkDeleteFunc(id) {
	url = "<?=base_url('admin/userBulkDelete')?>";
